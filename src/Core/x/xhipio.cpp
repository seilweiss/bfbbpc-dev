...
static HIPLOADDATA* HIPLCreate(const char* filename, char* dblbuf, int32 bufsize);
static void HIPLDestroy(HIPLOADDATA* lddata);
static uint32 HIPLBaseSector(HIPLOADDATA* lddata);
static int32 HIPLSetBypass(HIPLOADDATA* lddata, int32 enable, int32 use_async);
...
static en_READ_ASYNC_STATUS HIPLPollRead(HIPLOADDATA* lddata);
