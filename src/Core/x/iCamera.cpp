...
#define DEFAULT_NEAR_CLIP 0.05f
#define DEFAULT_FAR_CLIP 400.0f

float32 sCameraNearClip = DEFAULT_NEAR_CLIP;
float32 sCameraFarClip = DEFAULT_FAR_CLIP;
...
{
    sCameraNearClip = (nearPlane <= 0.0f) ? DEFAULT_NEAR_CLIP : nearPlane;
    sCameraFarClip = (farPlane <= 0.0f) ? DEFAULT_FAR_CLIP : farPlane;
}
...
STUB void iCameraUpdateFog(RwCamera*, iTime t)
{
}
...
void iCameraSetFogRenderStates()
{
    ...
    {
        bite_me =
            (pFogParams->fogcolor.alpha << 24) |
            (pFogParams->fogcolor.red << 16) |
            (pFogParams->fogcolor.green << 8) |
